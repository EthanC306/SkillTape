// What "due" means — the rule plans/fsrs_ui.md Phase 4 asks to be decided once
// and applied in one place, and the one that has to hold for the home screen to
// be honest: **GET /counts and GET /queue must always agree.**
//
// This exists because the rule shipped wrong the first time. "Due" was defined
// as `due_on <= end of local day` for everything, which is right for review
// cards and badly wrong for learning cards, whose steps are 1 and 10 MINUTES —
// a card reviewed seconds ago was immediately "due" again, so the home count
// never fell as you worked. Drilling 8 items and watching the number sit at 35
// is what turned it up. Both halves of the fixed rule are pinned below.
//
...
item("rev-later-today", 2, STATE.REVIEW, +6 * HOUR); //      due: day-granular, so today counts
...
test("the learning bucket ordering puts elapsed steps ahead of overdue reviews", async () => {
  ...
  const lastLearningAt = q.map((i) => i.id).reduce((last, id, i) => (/^(learn|relearn)-/.test(id) ? i : last), -1);
  assert.ok(
...
test("the daily new-item limit caps how many unseen items a session introduces", async () => {
  await fetch(`${base}/settings`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dailyNewLimit: 1 }),
  });
  ...
  await fetch(`${base}/settings`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dailyNewLimit: 10 }),
  });
});
