...
export default {
  ...
  cards: [
    ...
    },
    {
      ...
      body:
        "For a relation R from A to B, the set A you start from is the **domain** and the set B you land in is the **co-domain**. These are the declared sets, so every relation from A to B has domain A and co-domain B, whether or not each element actually gets used. Example: if R goes from A = {1, 2} to B = {1, 2, 3}, its domain is {1, 2} and its co-domain is {1, 2, 3}, even if some element of B is never hit.",
    },
    {
    ...
    },
    {
      ...
      body:
        "When are two functions the **same**? Not when their formulas look alike, but when they have the **same domain**, the **same co-domain**, and produce the **same output** for **every** input. So a function is really its input-output behavior, formula aside. Example: on ℝ, f(x) = |x| and g(x) = √(x²) are written differently but agree at every x, since √(x²) always equals |x|, so **f = g**.",
    },
  ],
  questions: [
    {
