  ...
  items: [
    ...
    makeItem({
      ...
      criteria: [
        ...
        "Defines the private helper nextIndex(size_t index) const returning (index + 1) % CAPACITY, where the modulo is what makes the array circular",
        "Public interface: Queue(), push(const Item&), pop(), peek(), size(), isEmpty()",
        "size() returns numItems and isEmpty() tests numItems == 0, so emptiness is tracked by the counter, not by comparing front and rear",
    ...
    makeItem({
      ...
      criteria: [
        ...
        "Pops from front while push appends at rear, and that opposite-ends pairing is what makes it FIFO",
      ],
      timeBudgetSec: 150,
      provenance: {
        sourceId: "cpp-slides-05.2-queues",
