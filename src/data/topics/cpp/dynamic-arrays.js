  ...
  cards: [
    {
      ...
      body:
        "An array name and a pointer to its element type work interchangeably. Given `int *intPtr;` and `int numbers[10];`, the assignment `intPtr = numbers;` makes intPtr **point to the first element** of numbers. From there, `intPtr[2] = 99;` stores 99 in the **third slot** (index 2), just like `numbers[2]` would.",
    },
    {
    ...
    },
    {
      ...
      body:
        "`delete[]` only works correctly if the pointer still points at the **start of the block** `new` gave you. If you shift it first with `chArray += 5;` and then call `delete[] chArray;`, the runtime tries to free 10 elements starting from the **sixth position**, which can **corrupt memory or crash** the program. Never delete through a pointer that has been moved from its original allocation.",
    },
    {
      heading: "Pointers and function parameters",
      body:
        "Because arrays and pointers are compatible, a function that expects an array can just take a pointer: `void someFunction(int *intPtr);` accepts either a **plain pointer or an array name**, since the array decays to a pointer to its first element when passed.",
    },
    {
      heading: "Pointers as reference parameters",
      body:
        "Pointers give you another way to write a swap-by-reference function: `void swap(int *intPtr1, int *intPtr2)` dereferences both parameters to exchange the values they point to. The caller passes **addresses**, as in `swap(&x, &y);`, so the function can modify x and y directly, the same job reference parameters do.",
    },
    {
    ...
    },
    {
      ...
      body:
        "Adding an integer to a pointer doesn't add that many bytes. `intPtr + i` moves the address by `i * sizeof(int)`, so the arithmetic automatically **accounts for the pointed-to type's size**. That's what lets `*(intPtr + i)` walk through a dynamic array one element at a time, the same as `intPtr[i]`.",
    },
  ],
  questions: [
    {
