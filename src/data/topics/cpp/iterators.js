...
export default {
  ...
  cards: [
    ...
    },
    {
      ...
      body:
        "**Input** iterators let you retrieve data from the object. **Output** iterators let you modify it but not read it back. **Forward** iterators do input and output but move in one direction only. **Bidirectional** iterators add everything above plus movement in either direction. **Random** access iterators go further still, allowing access by **index** as in `[i]`. The categories **nest**, with input and output at the center, wrapped by forward, then bidirectional, then random access, so each level can do everything the level inside it can.",
    },
    {
      heading: "An iterator for your own List class",
      body:
        "Once List provides an iterator you use it exactly like an STL one. Declare it with **List <int>::iterator it;**, set it to **begin()**, compare it against **end()**, and advance it with **++it**. Or skip the long type name and write **auto**, which works out the type from the **assignment**.",
      code:
    ...
    },
    {
      ...
      body:
        "The three **using** lines publish the type information C++ asks for. The constructor takes the node to start on and **defaults to nullptr**, and that default is what makes a plain `Iterator()` mean **one past the end**.",
      code:
  ...
  flashcards: [
    ...
    },
    {
      ...
      back:
        "They differ only in return type, with identical parameter lists and identical const qualifier, and the return type is not part of the signature.",
    },
    {
    ...
    },
    {
      ...
      back: "`nullptr` carries no link back, so there is no node to step backwards from.",
    },
    {
  ...
  items: [
    ...
    makeItem({
      ...
      expected:
        "Containers may have complex types and data structures, and sometimes it's not easy to reference elements using an integer index, as with linked lists. Iterators solve this problem by creating a standard way to navigate container elements regardless of their complexity or data structures.",
      criteria: [
    ...
    makeItem({
      ...
      criteria: [
        ...
        "Marked const, since dereferencing doesn't move or modify the iterator",
      ],
      timeBudgetSec: 120,
      provenance: {
        sourceId: "cpp-slides-04.2-iterators",
    ...
    makeItem({
      ...
      expected:
        "// overload prefix ++ operator as in ++it\nIterator &operator++() {\n    current = current->getNext();\n    return *this;\n}",
      criteria: [
        "Takes no parameters, and that empty list is what makes it the prefix form",
    ...
    makeItem({
      ...
      criteria: [
        ...
        "Advances with current = current->getNext()",
        "Returns original by value, not by reference, since the copy is the pre-increment state and must outlive the call",
      ],
      timeBudgetSec: 150,
      provenance: {
        sourceId: "cpp-slides-04.2-iterators",
    ...
    makeItem({
      ...
      criteria: [
        ...
        "Marked const, since comparing doesn't modify either iterator",
    ...
    makeItem({
      ...
      criteria: [
        "Returns bool and takes the other Iterator as a const parameter",
        "Compares the stored pointers with !=, as current != other.current",
        "Marked const",
    ...
    makeItem({
      ...
      criteria: [
        "Takes const Node<ItDataType> *initial, and that const is what separates this from the plain Iterator constructor",
