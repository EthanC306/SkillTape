  ...
  questions: [
    {
      ...
      prompt:
        "On the deck's table of ordered data, the array and the linked list differ on exactly one of the three operations. Which one, and in whose favor?",
      choices: [
        "Search, in the array's favor: O(log2n) against O(n)",
        "Insert, in the linked list's favor: O(1) against O(n)",
        "Remove, in the linked list's favor: O(1) against O(n)",
        "Search, in the linked list's favor: O(1) against O(log2n)",
      ],
      answer: 0,
      explanation: "Both structures are O(n) on insert and on remove, so search is the only row where their times differ — which is why the deck's advantage column has to talk about resizing rather than complexity.",
    },
    {
      id: "trees-q02",
      verifiedByHuman: false,
      prompt:
        "In the deck's example tree, what is the depth of node 10, and what is the height of the tree by the maximum-depth-of-a-leaf definition?",
      code:
        "        50\n      /    \\\n    40      60\n   /  \\    /  \\\n  35  45  55  70\n /              \\\n10              80",
      choices: [
        "Depth 3 and height 4",
        "Depth 4 and height 4",
        "Depth 3 and height 3",
        "Depth 2 and height 3",
      ],
      answer: 2,
      explanation: "Depth counts steps from the root, so 50 to 40 to 35 to 10 is 3, and no leaf sits deeper — which makes the height 3 under that definition and 4 under the deck's other one.",
    },
    {
      id: "trees-q03",
      verifiedByHuman: false,
      prompt:
        "A structure has one node with no parent, every other node has exactly one parent, and one node has three children. By the deck's definitions, what is it?",
      choices: [
        "A binary tree, since the third child can be held in one of the subtrees",
        "Not a tree at all — three children breaks the one-parent rule",
        "A Full Binary Tree, since every non-leaf has more than one child",
        "A tree, but not a binary tree",
      ],
      answer: 3,
      explanation: "The parent rules are what make something a tree; \"binary\" is the separate, stricter promise that no node has more than a left and a right child.",
    },
    {
      id: "trees-q04",
      verifiedByHuman: false,
      prompt: "Which of these trees is Full by the deck's definition?",
      choices: [
        "Every level except the last is completely filled, and the last fills in from the left",
        "A root with two children, both of them leaves",
        "A root whose left child has two leaf children and whose right child is a leaf",
        "A root with a single child, which itself has two leaf children",
      ],
      answer: 1,
      explanation: "Both halves of the definition have to hold at once, and the third option fails only the leaf-depth half — its leaves sit at depth 2 and depth 1.",
    },
    {
      id: "trees-q05",
      verifiedByHuman: false,
      prompt: "Given this constructor, what does `data` hold after `BNode<int> n;`?",
      code:
        "BNode(DataType newData = DataType(),\n      BNode* newLeft = nullptr, BNode* newRight = nullptr){\n    data = newData;\n    left = newLeft;\n    right = newRight;\n}",
      choices: [
        "Garbage — no argument was given, so data is never assigned",
        "nullptr, the same default the two pointer parameters get",
        "A compile error: an int has no default constructor to call",
        "0 — DataType() value-initializes, and for int that is zero",
      ],
      answer: 3,
      explanation: "`DataType()` is a value-initialization, which is defined for the built-in types too, so the default it supplies is a real zero rather than an uninitialized slot.",
    },
    {
      id: "trees-q06",
      verifiedByHuman: false,
      prompt:
        "`insert` is `{ treeInsert(root, newData); }` and treeInsert takes `BNode<DataType>* &root`. What does that reference bind to on the first call, and why does it matter?",
      choices: [
        "A copy of the root member, so an insert into an empty tree is silently lost",
        "root->left or root->right, chosen by comparing newData before the call is made",
        "The class's own root member, so an insert into an empty tree can set the root itself",
        "A fresh node the wrapper allocates, which treeInsert then fills in",
      ],
      answer: 2,
      explanation: "The base case assigns through that reference, so it has to name the caller's actual pointer — the member on the first call, and a parent's left or right on every call after.",
    },
    {
      id: "trees-q07",
      verifiedByHuman: false,
      ...
      choices: [
        ...
        "10 35 40 45 55 70 80 60 50",
        "10 35 45 40 55 80 70 60 50",
      ],
      answer: 3,
      explanation: "A node prints only after both of its subtrees, so 40 comes out after 35 and 45 rather than between them — the trap in the near-miss option.",
    },
    {
      id: "trees-q08",
      verifiedByHuman: false,
      prompt:
        "You are told only that some traversal of the deck's example tree printed 10 first and 50 last. Which traversals are consistent with that?",
      choices: [
        "Postorder only",
        "Inorder only",
        "Inorder or postorder — both reach 10 first",
        "Postorder or level order — both end at the root",
      ],
      answer: 0,
      explanation: "Inorder and postorder both bottom out at 10 first, but only postorder holds the root back to the very end; inorder finishes at 80 and level order starts at 50.",
    },
    {
      id: "trees-q09",
      verifiedByHuman: false,
      prompt: "Which traversal order would you use to delete every node of a tree, and why?",
      choices: [
        "Postorder: a node is freed only after both of its subtrees, so no pointer is followed after it is freed",
        "Preorder: freeing the root first stops the children from being reached and freed twice",
        "Inorder: the left subtree is freed before the node and the right after, which balances the work",
        "Any of the three: delete does not disturb the left and right pointers of the node being freed",
      ],
      answer: 0,
      explanation: "Freeing a node destroys the only two pointers that lead to its children, so anything that processes a node before its subtrees loses them.",
    },
    {
      id: "trees-q10",
      verifiedByHuman: false,
      prompt:
        "Someone changes treeSize's base case to `return 1;`. What does size() then report for the deck's 9-node example tree?",
      code:
        "size_t BSTree<DataType>::treeSize(BNode<DataType>* root) {\n    if(root == nullptr){\n        return 1;   // was 0\n    }\n    return 1 + treeSize(root->left) + treeSize(root->right);\n}",
      choices: [
        "10 — one for each nullptr the recursion lands on",
        "19",
        "18 — every node is now counted twice",
        "9 — the extra counts cancel against the missing ones",
      ],
      answer: 1,
      explanation: "A tree of n nodes has exactly n+1 null child slots, so the broken base case fires 10 times and adds 10 to the 9 real nodes.",
    },
    {
      id: "trees-q11",
      verifiedByHuman: false,
      prompt: "Why is inorder on a BST guaranteed to print sorted order rather than merely happening to?",
      choices: [
        "Because insert places each value into sorted position as it walks down",
        "Because inorder visits the nodes in order of increasing depth from the root",
        "Because the traversal rule and the BST storage rule state the same ordering twice",
        "It is not guaranteed — the printed order depends on the order values were inserted",
      ],
      answer: 2,
      explanation: "Inorder says left, me, right, and the storage rule says left is smaller and right is larger, so every node necessarily prints between everything below it and everything above it.",
    },
    {
      id: "trees-q12",
      verifiedByHuman: false,
      ...
      choices: [
        "So an equal value takes the right branch and is stored as a duplicate leaf",
        "Because a bare else is not legal C++ directly after an if with a body",
        "Because DataType must exercise both operator< and operator> for the template to compile",
        "So an equal value satisfies neither test and the duplicate is ignored",
      ],
      answer: 3,
      explanation: "A bare else would send an equal value down the right subtree until it hit nullptr and inserted it, quietly breaking the storage rule that every node's value is unique.",
    },
    {
      id: "trees-q13",
      verifiedByHuman: false,
    ...
    },
    {
      ...
      verifiedByHuman: false,
      prompt:
        "treeSearch and treeSize walk the same tree, but the deck puts search at O(log2n) and size is O(n). What single property accounts for the difference?",
      code:
        "size_t BSTree<DataType>::treeSize(BNode<DataType>* root) {\n    if(root == nullptr){\n        return 0;\n    }\n    return 1 + treeSize(root->left) + treeSize(root->right);\n}",
      choices: [
        "treeSize makes two recursive calls per node against treeSearch's one, which costs it one extra level of depth",
        "treeSearch compares at each node and drops the subtree that cannot hold the target; treeSize never compares",
        "treeSize visits every node twice, once on the way down and once to add its 1 on the way back",
        "treeSearch is O(log2n) only because it is measured on a balanced tree, while treeSize is measured on an unbalanced one",
      ],
      answer: 1,
      explanation: "O(log2n) is never a property of the tree by itself — it is what a comparison buys you, and a function that makes none has to touch everything.",
    },
    {
      id: "trees-q15",
      verifiedByHuman: false,
      prompt:
        "On the deck's final table, which advantage do the linked list and the binary search tree share, and what still separates them?",
      choices: [
        "No resizing and no copying; the BST beats the linked list on all three operations",
        "No resizing and no copying; the BST is faster only on search",
        "Fast access; the BST is the slower of the two on a small dataset",
        "No resizing and no copying; the linked list still beats the BST on insert",
      ],
      answer: 0,
      explanation: "Both avoid the array's resizing cost, but the linked list pays O(n) on all three operations where the BST pays O(log2n) — which is why the deck's disadvantage column for the tree has to reach for small datasets instead.",
    },
  ],
