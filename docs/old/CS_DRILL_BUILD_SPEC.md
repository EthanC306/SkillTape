...
**Purpose:** A source-grounded quiz and spaced-repetition system for Ethan's CS curriculum. Every question traces back to a verbatim excerpt from his own course materials (zyBooks, the course's slides, lab handouts, WebAssign problems). No facts originate from the model.
...
```sql
CREATE TABLE topics (
  id            TEXT PRIMARY KEY,     -- 'cpp.linked-lists.doubly'
...
### Phase 1 — Scaffold
`package.json`, `schema.sql`, `curriculum.yaml` loader, DB init, `cs-drill --help`. Seed `curriculum.yaml` with the cpp and Discrete Structures topic trees.
**Done when:** `npm run init` creates a valid DB and `npm run audit` passes on an empty bank.

### Phase 2 — Ingest + pilot
Source parser (frontmatter + anchored sections). Then hand-build 8 atoms and 8 items for one section — `sources/cpp/lab03-doubly-linked.md` — entirely by hand, no model generation.
...
`.claude/skills/item-writer/SKILL.md`. Atoms → items, honoring §6 quotas. Must emit `criteria` as a checklist so self-grading isn't vibes. Pulls C++ style from the existing `cpp-tutor` conventions: `nullptr`, `using namespace std;`, no `#include <string>`, attached opening brace, `} // main`.
...
- curriculum.yaml with topic trees for cpp (pointers, dynamic arrays,
