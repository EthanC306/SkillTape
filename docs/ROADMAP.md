...
`sources/` · `references/ou-cpp-conventions.md` · `.claude/commands/` · provenance on any card or item · `examWeight` on `discrete` topics · attempts log · scheduler · `DrillView` · exam simulator · reporting · the entire platform track.
...
- `discrete` course topics have **no `examWeight` yet** — this midterm was CS2401 (`cpp`) only; CS3000 gets its own pass once there's exam data for it. The "every topic" done-when criterion is not literally met.
