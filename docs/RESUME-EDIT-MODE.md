...
- **`Shell.jsx` remounts `App` on every course-tab switch** (`<App key="cpp" …>`). That's why the topics fetch is cached at module level in `src/api/client.js` rather than in a hook — without it, every tab switch refetches the whole bank.
