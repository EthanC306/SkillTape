...
- **`subtitle`** — course code plus a short scope line. C++ style: `"Data Structures — destructors, copy constructors & operator="`. Discrete style: `"CS3000 — §2.4"`.
...
3. `src/Shell.jsx` — **the bottom tab bar is hardcoded**, not generated from `COURSES`. Tabs are keyed by their own ids (`"cpp"`, `"cs3000"`) which are *not* the course ids, and each branch renders `<App key="…" course="…" />`. A new course needs a branch here or it will never be reachable.
