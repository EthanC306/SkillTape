...
| `server/routes/progress.js` | `POST /api/attempts`, `GET /api/progress`, `GET /api/questions/stats` (all open). See § 6. |
...
- For **every** topic, on **every** run — with or without `--reset` — it unconditionally runs `DELETE FROM cards/flashcards WHERE topic_id = ?` for that topic and reinserts from the current contents of the `.js` file. This delete-and-reinsert is *not* gated behind the `--reset` flag. It is safe only because nothing references a card or a flashcard row.
- **`items` and `questions` are the exceptions, and deliberately so** (`items` fixed 2026-08-09, `questions` 2026-08-10). Both are *upserted* (`ON CONFLICT(id)` / `ON CONFLICT(stable_id) DO UPDATE`) and then pruned of ids that no longer appear in the `.js` file — never cleared wholesale. This section predates both tables, and the delete-and-reinsert pattern described above was silently catastrophic once applied to them:
  - `server/db.js` sets `foreign_keys = ON`, and `item_review_state`, `item_attempts` and `item_suspensions` all declare `item_id REFERENCES items(id) ON DELETE CASCADE`. So a plain reseed **destroyed every user's FSRS scheduling state, the entire attempt log, and all suspensions** — for every item, including ones whose content hadn't changed — and reinserting the same id did not bring them back. Editing one prompt cost you your whole study history.
  - `questions.id` is `AUTOINCREMENT`, so re-inserting handed every question a **new** id, and `attempts.question_id` is `ON DELETE SET NULL`. Every reseed therefore blanked the record of *which question* each past attempt had answered — a quieter failure than the `items` one, and it had already wiped all 38 attempt rows in the author's database before it was noticed. The fix (`docs/STABLE_QUESTION_IDS.md`) is an authored `stable_id` on the question itself (`"bigo-q03"`, § 4.1 of `docs/AUTHORING.md`), which is what `attempts.question_stable_id` records.

  Do not "simplify" either pass back into a `DELETE`. `choices` is still replaced wholesale per question, which stays safe because nothing references a choice row.
- Each question also carries a `revision`, bumped by `seed.js` when a SHA-256 of its **graded** fields (`prompt`, `code`, `choices`, `answer`) changes — never by hand. `explanation`, `tag` and `figure` are excluded, so cosmetic edits are free. Each attempt records the revision it was answered at, and the seed prints every question it bumped.
- `--reset` **prunes rather than wipes**: content rows whose ids no longer appear in the curriculum are deleted (their history describes something that no longer exists), and everything still authored keeps its identity. It used to be a blunt `DELETE FROM topics`, which cascaded through `attempts.topic_id` and `items.topic_id` and silently took the entire quiz history, every FSRS schedule and every suspension with it — the same bug class as above, one flag away from a routine reseed. Net content state after a reset is what it always was; to genuinely start from nothing, delete `db/skilltape.db`.
- `users`, `sessions`, and `attempts` are never written by `seed.js`, `--reset` or not.

**Consequence: any `npm run db:seed` run — reset or not — overwrites that topic's cards and flashcards with whatever is currently in the `.js` file, discarding any Edit Mode changes made in the database that were never backported to the file.** Edit Mode and re-seeding are mutually destructive on the same topic; there is no merge, and whichever happens last wins. (Edit Mode can't reach questions or items — `topics.js` only exposes `PUT .../cards` and `PUT .../flashcards` — so the upserted tables aren't part of this conflict.)
...
`GET /api/progress` derives the older UI shape — `{ [topicId]: { best, total, runs, history } }`, unchanged since `useProgress.js`'s `localStorage` days — from the per-item rows in SQL (`GROUP BY run_id, topic_id`), so `Home.jsx`, `HistoryModal.jsx`, and `TopicView.jsx` needed no changes when progress moved off `localStorage`. A one-time importer in `useProgress.js` migrates any pre-existing `localStorage["cppquiz:progress"]` history into `attempts` on first load (with no question id, since the old format only ever recorded booleans), then deletes the legacy key.

`GET /api/questions/stats` is the per-question view the `run_id` grouping above cannot give: one row per authored question — `{ stableId, topicId, revision, prompt, attempts, correct, attemptsAtCurrentRevision, correctAtCurrentRevision, lastAnsweredAt }` — aggregated in SQL. It `LEFT JOIN`s *from* `questions`, so a question nobody has answered appears with `attempts: 0` rather than vanishing; "never seen" and "seen and always missed" are opposite signals and must not look alike. The `…AtCurrentRevision` pair is what makes a rewrite legible: a question answered correctly and then made harder reads as `correct: 1` but `correctAtCurrentRevision: 0`, because the old success no longer speaks for the new wording. This endpoint is the point of the whole stable-id change — it answers "which specific MCQs am I actually getting right, so I can rewrite those harder and leave the rest alone."

`POST /api/attempts` is a single transaction per run — every row of a sitting commits together, so a failure partway through never leaves a half-recorded quiz in the history. Its `questionStableId` is validated but an *unknown* value is demoted to `NULL` rather than 400'd: rejecting would discard a whole finished run (`useProgress.js` only logs the failure), and the likeliest cause is benign — a reseed deleting a question while the app still has the old bank loaded.
